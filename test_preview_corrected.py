    ...
    for i, row in enumerate(formatted_data):
        ...
        if i < len(test_references):
            ...
            if original_ref.lower() in ['c', 'cr', 'credit']:
                expected_amount_sign = "negative"
                expected_reference = "C"
                expected_display = f"negative amount / C"
            else:  # Debit cases
                expected_amount_sign = "positive"
                expected_reference = "D"
                expected_display = f"positive amount / D"
            
            # Check if actual matches expected
            try:
                amount_value = float(amount)
                if expected_amount_sign == "negative":
            ...
            print(f"{i+1:<3} | {original_ref:<12} | {amount:<15} | {reference:<12} | {expected_display} {status}")
            
            if not (amount_correct and reference_correct):
                all_correct = False
                if not amount_correct:
                    print(f"    ❌ Expected {expected_amount_sign} amount, Got: {amount}")
